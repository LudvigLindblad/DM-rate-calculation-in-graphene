...
mu=(ct.me*ct.mchi)/(ct.me+ct.mchi);

R_1=[ct.a, 0, 0];      #Nearest-neighbor vectors
R_2=[-ct.a/2, 3**(1/2)*ct.a/2, 0];
R_3=[-ct.a/2, -3**(1/2)*ct.a/2, 0];



def fl(lx, ly, lz):  #Phase factor
    l=[lx, ly, lz];
...
def createP_sigma(lx, ly, band): 
    ...
    nrm3=1/mp.sqrt(mp.conj(C[0])*C[0]+mp.conj(C[1])*C[1]+mp.conj(C[2])*C[2]+3*(mp.conj(C[3])*C[3]+mp.conj(C[4])*C[4]+mp.conj(C[5])*C[5])+2*mp.re((mp.conj(C[0])*C[3]*pss+mp.conj(C[0])*C[4]*psx+mp.conj(C[0])*C[5]*psy+mp.conj(C[1])*C[3]*psx+mp.conj(C[1])*C[4]*pxx+mp.conj(C[1])*C[5]*pxy+mp.conj(C[2])*C[3]*psy+mp.conj(C[2])*C[4]*pxy+mp.conj(C[2])*C[5]*pyy)*(mp.exp(1j*mp.fdot([lx, ly, 0], R_1))+mp.exp(1j*mp.fdot([lx, ly, 0], R_2))+mp.exp(1j*mp.fdot([lx, ly, 0], R_3)))));
   
    return lambda kx, ky, kz: f(kx, ky, kz)*nrm3;  
...
def E_pi_minus(lx, ly):
    ...
    return -1*t*fle/(1+s*fle);     #Returns negative value

def E_sigma(lx, ly, band):
    E=table.Ematrix(lx, ly, band);
    return -1*ct.q*E;   #Returns negative value
...
def F_DM(qr, fdm):
    ...
    if fdm==1:
        ...
        return ct.sigma_e*qr;    # sigma_e not needed, the scale is arbitrary.         !!!!!  Fabian formulera om?
    elif fdm==3:
